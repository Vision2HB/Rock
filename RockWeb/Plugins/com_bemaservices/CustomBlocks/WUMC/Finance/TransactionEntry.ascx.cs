...
/*
 * Additional Features:
 * - FE1) Changed personal info to show firstname & lastname instead of nickname & lastname
 * - FE2) Added Title fields
 */

namespace RockWeb.Plugins.com_bemaservices.CustomBlocks.WUMC.Finance
{
    ...
    [Category( "BEMA Services > WUMC > Finance" )]
  ...
  },
  {
    ...
    public partial class TransactionEntry : Rock.Web.UI.RockBlock
    {
        ...
        private void SetControlOptions()
        {
            ...
            /* BEMA.FE2.Start */
            dvpTitle.Visible = person == null;

            if ( person != null )
            {
                dvpTitle.SelectedDefinedValueId = person.TitleValueId;
            }
            /* BEMA.FE2.End */
        ...
        private void ShowPersonal( Person person )
        {
            ...
            {
                /* BEMA.FE2.Start */
                var nameText = person.FullName;
                if ( person.TitleValueId.HasValue )
                {
                    var personTitleValue = DefinedValueCache.Get( person.TitleValueId.Value );
                    if ( personTitleValue != null )
                    {
                        nameText = string.Format( "{0} ", personTitleValue.Value ) + nameText;
                    }
                }

                txtCurrentName.Text = nameText;
                /* BEMA.FE2.End */
        ...
        private bool ProcessPaymentInfo( out string errorMessage )
        {
            ...
            /* BEMA.FE2.Start */
            var nameText = paymentInfo.FullName.Trim();
            if ( dvpTitle.SelectedDefinedValueId.HasValue )
            {
                var personTitleValue = DefinedValueCache.Get( dvpTitle.SelectedDefinedValueId.Value );
                if ( personTitleValue != null )
                {
                    nameText = string.Format( "{0} ", personTitleValue.Value ) + nameText;
                }
            ...
            tdNameConfirm.Description = nameText;
            /* BEMA.FE2.End */
        ...
        private void ShowSuccess( GatewayComponent gatewayComponent, Person person, PaymentInfo paymentInfo, PaymentSchedule schedule, FinancialPaymentDetail paymentDetail, RockContext rockContext )
        {
            ...
            tdNameReceipt.Description = nameText;
            /* BEMA.FE2.End */
