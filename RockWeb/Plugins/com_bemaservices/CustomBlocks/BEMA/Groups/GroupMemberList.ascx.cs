 ...
 * 
 * Additional Features:
 * - FE1) Added ability to specify Merge Template route
 */
namespace RockWeb.Plugins.com_bemaservices.Groups
{
    ...
    /* BEMA.FE1.Start */
    [TextField(
        "Alternate Merge Template Route",
        Key = BemaAttributeKey.AlternateMergeTemplateRoute,
        Description = "An alternate route for the block to navigate to in order to generate merge templates. Use the format '~/MergeTemplate/{0}'.",
        DefaultValue = "",
    ...
    public partial class GroupMemberList : RockBlock, ISecondaryBlock, ICustomGridColumns
    {
        ...
        {
            public const string AlternateMergeTemplateRoute = "AlternateMergeTemplateRoute";
        }
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            if ( !( groupId == 0 && groupGuid == Guid.Empty ) )
            {
                ...
                if ( _group != null && _group.IsAuthorized( Authorization.VIEW, CurrentPerson ) )
                {
                    ...
                    /* BEMA.FE1.Start */
                    if ( GetAttributeValue( BemaAttributeKey.AlternateMergeTemplateRoute ).IsNotNullOrWhiteSpace() )
                    {
                        gGroupMembers.MergeTemplatePageRoute = GetAttributeValue( BemaAttributeKey.AlternateMergeTemplateRoute );
                    }
                    /* BEMA.FE1.End */
