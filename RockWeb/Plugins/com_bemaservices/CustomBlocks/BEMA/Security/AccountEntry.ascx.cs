 ...
 * 
 * Additional Features:
 * - FE1) Added google reCaptcha integration
 */
...
namespace RockWeb.Plugins.com_bemaservices.CustomBlocks.BEMA.Security
{
    ...
    /* BEMA.FE1.Start */
    [TextField( "Recaptcha Site Key", "The Site or Public Key From the Google reCAPTCHA Page", false, null, "BEMA Additional Features", 0, BemaAttributeKey.ReCAPTCHAPublicKey ) ]
    [TextField( "Recaptcha Secret Key", "The Secret or Private Key From the Google reCAPTCHA Page", false, null, "BEMA Additional Features", 0, BemaAttributeKey.ReCAPTCHAPrivateKey )]
    [TextField( "Recaptcha Version", "The API version of reCaptcha.  Currently only v1 and v2 are supported.", false, "2", "BEMA Additional Features", 0, BemaAttributeKey.ReCAPTCHAVersion )]
    /* BEMA.FE1.End */
    ...
    public partial class AccountEntry : Rock.Web.UI.RockBlock
    {
        ...
        private static class BemaAttributeKey
        {
            ...
            public const string ReCAPTCHAVersion = "ReCAPTCHAVersion";
        }
        /* BEMA.End */
        #region Fields
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            /* BEMA.FE1.Start */
            if( !string.IsNullOrWhiteSpace( GetAttributeValue( BemaAttributeKey.ReCAPTCHAPublicKey ) )
                && !string.IsNullOrWhiteSpace( GetAttributeValue( BemaAttributeKey.ReCAPTCHAPrivateKey ) )
                && !string.IsNullOrWhiteSpace( GetAttributeValue( BemaAttributeKey.ReCAPTCHAVersion ) ) )
            {
                Recaptcha1.PrivateKey = GetAttributeValue( BemaAttributeKey.ReCAPTCHAPublicKey );
                Recaptcha1.PublicKey = GetAttributeValue( BemaAttributeKey.ReCAPTCHAPrivateKey );
                Recaptcha1.ApiVersion = GetAttributeValue( BemaAttributeKey.ReCAPTCHAVersion );
        ...
        protected void btnUserInfoNext_Click( object sender, EventArgs e )
        {
            ...
            if ( Page.IsValid )
            {
                   ...
                   && !string.IsNullOrWhiteSpace( GetAttributeValue( BemaAttributeKey.ReCAPTCHAVersion ) ) )
                {
                    ...
                    {
                        ShowErrorMessage( "Captcha cannot be empty." );
                        return;
                    }
                    ...
                    {
                        ShowErrorMessage( "Incorrect captcha response." );
                        return;
                    }

                    else if ( result != RecaptchaVerificationResult.Success )
                    {
                        ShowErrorMessage( "An Error occured with Captcha, please contact the site's administrator" );
                        ExceptionLogService.LogException( new Exception( "An error occured with Recaptcha: " + Recaptcha1.Response ) );
                        return;
                    }
