 ...
 * - FE4) Added Ability to use a checkboxlist to set transaction comments
 * 
 */

namespace Rock.Plugins.com_bemaservices.CustomBlocks.FLCC.Finance
{
    /// <summary>
    /// Add a new one-time or scheduled transaction
    /// </summary>
    [DisplayName( "Transaction Entry" )]
    [Category( "BEMA Services > FLCC > Finance" )]
    ...
    /* Bema.FE4.Start */
    [ValueListField( "Partner Options", "The Partner Options available to be selected.", false, "", "", "", "", "BEMA Additional Features", 38, BemaAttributeKey.PartnerOptions )]
    [TextField( "Partner Options Label", "The label to use for the partner options checkboxlist.", false, "Preferred Partner Options", "BEMA Additional Features", 39, BemaAttributeKey.PartnerOptionsLabel )]
    /* Bema.FE4.End */
    ...
    public partial class TransactionEntry : Rock.Web.UI.RockBlock
    {
        ...
        private static class BemaAttributeKey
        {
            ...
            public const string PartnerOptions = "PartnerOptions";
            public const string PartnerOptionsLabel = "PartnerOptionsLabel";
        }
        #endregion
        /* BEMA.End */
  ...
  },
  {
    ...
    'AmountFormatted': '$10.00'
        ...
        private void SetControlOptions()
        {
            ...
            /* BEMA.FE4.Start */
            var checkBoxListOptions = GetAttributeValue( BemaAttributeKey.PartnerOptions ).SplitDelimitedValues( false );
            if ( checkBoxListOptions.Any() )
            {
                cblPartnerOptions.Items.Clear();
                foreach ( var checkBoxItem in checkBoxListOptions )
                {
                    cblPartnerOptions.Items.Add( new ListItem( checkBoxItem ) );
                }
                cblPartnerOptions.Visible = true;

                if ( GetAttributeValue( BemaAttributeKey.PartnerOptionsLabel ).IsNotNullOrWhiteSpace() )
                {
                    cblPartnerOptions.Label = GetAttributeValue( BemaAttributeKey.PartnerOptionsLabel );
                }
            }
            /* BEMA.FE4.End */
