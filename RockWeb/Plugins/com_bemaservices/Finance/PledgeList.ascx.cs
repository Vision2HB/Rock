 ...
 */
namespace RockWeb.plugins.com_thewoodlandsumc.Finance
{
    [DisplayName( "Pledge List" )]
    [Category( "The Woodlands UMC Finance" )]
    ...
    public partial class PledgeList : RockBlock, ISecondaryBlock, ICustomGridColumns
    {
        ...
        private void GPledges_RowDataBound( object sender, System.Web.UI.WebControls.GridViewRowEventArgs e )
        {
            if ( e.Row.RowType == DataControlRowType.DataRow )
            {
                ...
                {
                    var cell = e.Row.Cells[8].Text = string.Empty;
                }

                if ( pledge.EndDate.ToShortDateString() == DateTime.MaxValue.ToShortDateString() )
                {
                    var cell = e.Row.Cells[9].Text = string.Empty;
                }
                //BEMA Custom Code Start
                ...
                var adjustedPedgeEndDate = pledge.EndDate.AddHours( 23 ).AddMinutes( 59 ).AddSeconds( 59 );

                //pledge.AmountGiven = 0;
                ...
                var cell2 = e.Row.Cells[5].Text = pledge.AmountGiven.ToString( "C" );

                var cell3 = e.Row.Cells[6].Text = ( pledge.AmountGiven > pledge.AmountPledged ) ? "0" : ( pledge.AmountPledged - pledge.AmountGiven ).ToString( "C" );

                if ( pledge.ModifiedByPersonAliasId != null )
                {
                    var cell4 = e.Row.Cells[11].Text = new PersonAliasService( rockContext ).Get( pledge.ModifiedByPersonAliasId.Value ).ToString();
                }
                //BEMA Custom Code End
            }
        }
