...
namespace RockWeb.Plugins.com_bemaservices.Finance
{
    ...
    [KeyValueListField( "Keyword Account Mapping", "This is the mapping for Kindrid Keywords to Rock Accounts", true, "UNDESIGNATED^1", "Kindrid Keyword", "Rock Account Id", "", "", "Account", 2 )]
        ...
        private ProcessStatus CreateTransaction( KindridTransaction transaction, int batchId )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                if ( !transaction.PreviouslyImported )
                {
                    ...
                    int accountId = _keywordAccountMappings.ContainsKey( transaction.Keyword.ToUpper() ) ? _keywordAccountMappings[transaction.Keyword.ToUpper()].AsInteger() : _defaultAccountId;

                    // Updating total counter
                    _totalAmount += transaction.GrossAmount;
