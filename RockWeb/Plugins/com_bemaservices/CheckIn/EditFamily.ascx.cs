...
namespace RockWeb.Plugins.com_bemaservices.Checkin
{
    ...
    public partial class EditFamily : CheckInEditFamilyBlock
    {
        ...
        private void CreateDynamicPersonControls( FamilyRegistrationState.FamilyPersonState familyPersonState, bool setValues )
        {
            ...
            var attributeCheckInGroupGuid = GetAttributeValue ( "Check-InGroupAttribute" ).AsGuidOrNull();
            var groupAttribute = new AttributeCache();
            GroupMember groupMember = null;

            if (attributeCheckInGroupGuid.HasValue)
            {
                groupAttribute = AttributeCache.Get ( GetAttributeValue ( "Check-InGroupAttribute" ).AsGuid () );
        ...
        private void EditGroupMember( Guid? groupMemberGuid )
        {
            ...
            var showMaritalStatus = GetAttributeValue ( "ShowMaritalStatusforAdult" ).AsBoolean ();
        ...
        private void ShowControlsForRole( bool isAdult )
        {
            ...
            // BEMA Custom
            pnMobilePhone.Visible = isAdult | GetAttributeValue ( "ShowMobileforChild" ).AsBoolean (); ;
            ...
            dpBirthDate.Required = (!isAdult & GetAttributeValue ( "RequireBirthdateforChild" ).AsBoolean ()) || ( isAdult & GetAttributeValue ( "RequireBirthdateforAdult" ).AsBoolean () );
            dpBirthDate.Visible = !isAdult | GetAttributeValue ( "ShowBirthdateforAdult" ).AsBoolean (); 
        ...
        protected void btnDonePerson_Click( object sender, EventArgs e )
        {
            ...
            // Faithbridge Custom for initials
        ...
        public FamilyRegistrationState.SaveResult SaveFamilyAndPersonsToDatabase( FamilyRegistrationState editFamilyState, int? kioskCampusId, RockContext rockContext )
        {
            ...
            var defaultMaritalStatus = GetAttributeValue ( "NewPersonMaritalStatus" ).AsGuidOrNull ();
            var showMaritalStatus = GetAttributeValue ( "ShowMaritalStatusforAdult" ).AsBoolean ();
            ...
            foreach ( var familyPersonState in editFamilyState.FamilyPersonListState.Where ( a => !a.IsDeleted ) )
            {
                ...
                if ( !familyPersonState.PersonId.HasValue )
                {
                    ...
                    {
                        var maritalStatusDefault = DefinedValueCache.Get ( GetAttributeValue ( "NewPersonMaritalStatus" ).AsGuid () );
