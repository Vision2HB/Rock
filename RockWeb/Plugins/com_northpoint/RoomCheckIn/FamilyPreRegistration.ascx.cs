...
//
//
// Edited by Sam Crisp for North Point Ministires
// Created from the core FamilyPreRegistration block to use a PersonId to pre-fill the form.
//
//
using System;
...
using com_northpoint.RoomCheckIn.Web.UI.Controls;
...
namespace RockWeb.Plugins.com_northpoint.RoomCheckIn
{
    ...
    [CustomDropdownListField( "Enroll Family", "How should the Enroll Family field be displayed (adds note to family)?", HIDE_OPTIONAL_REQUIRED, false, "Optional", "", 2 )]
    ...
    [AttributeField( Rock.SystemGuid.EntityType.GROUP, "GroupTypeId", Rock.SystemGuid.GroupType.GROUPTYPE_FAMILY, "Family Attributes", "The Family attributes that should be displayed. These are always required.", false, true, "", "", 3 )]
    ...
    [BooleanField( "PersonId Page Parameter", "Allow a URL parameter of FamilyId to pre-fill block information. DO NOT ENABLE ON PUBLIC SITE.", false, "", 4)]
...
following attribute keys defined, those attribute values will also be set: ParentIds, ChildIds, EnrollFamily.
", true, false, "", "", 8 )]
...
comma-delimited list of the person ids for each adult; 'ChildIds' which is a comma-delimited list of the person ids for each child; and 'EnrollFamily' which is 
the value entered for the Enroll Family field if it was displayed.
...
ORDER BY [Text]", false, "", "Child Relationship", 2, "CanCheckinRelationships" )]
    ...
    public partial class FamilyPreRegistration : RockBlock
    {
        ...
        /// The group members.
        /// </value>
        protected List<PreRegistrationChildNPM> Children { get; set; }
        ...
        protected override void LoadViewState( object savedState )
        {
            ...
            Children = ViewState["Children"] as List<PreRegistrationChildNPM> ?? new List<PreRegistrationChildNPM>();
...
",
        ...
        {
            var row = sender as PreRegistrationChildRowNPM;
        ...
        protected void btnSave_Click( object sender, EventArgs e )
        {
            if ( ValidateInfo() )
            {
                ...
                bool processRemovals = primaryFamily != null && ( GetAttributeValue( "AllowUpdates" ).AsBoolean() || GetAttributeValue( "PersonIdPageParameter" ).AsBoolean() );
                ...
                //North Point NPM BEMA
                //Save a description if none exists and Enroll My Family is selected
                if ( GetAttributeValue( "EnrollFamily" ) != "Hide" )
                {
                    //Add | Enroll Family: Y/N | to description; replace if there.
                    if ( primaryFamily.Description.IsNullOrWhiteSpace() )
                    {
                        primaryFamily.Description = "";
                    }
                    else
                    {
                        primaryFamily.Description = primaryFamily.Description.Replace( "Enroll Family: Y |", "" );
                        primaryFamily.Description = primaryFamily.Description.Replace( "Enroll Family: N |", "" );
                    }
                    primaryFamily.Description += "Enroll Family: " + ddlVisitingEnroll.SelectedValue.Substring(0,1) + " |";
                }
                ...
                foreach ( var child in Children )
                {
                    ...
                    //if NPM Profile Source, add 'New Family Registration Form'
                    if (GetAttributeValue( "ProfileSourceAttribute" ).AsGuidOrNull().HasValue)
                    {
                        ...
                        {
                            person.SetAttributeValue( key, "New Family Registration Form" );
                        }
                    }
                    ...
                    if ( shouldBeInPrimaryFamily )
                    {
                        ...
                        //NPM: if not child, add a relationship to adults in this family anyways
                        if ( ( child.RelationshipType ?? 0 ) != 0 )
                        {
                            foreach ( var adultId in adultIds )
                            {
                                groupMemberService.CreateKnownRelationship( adultId, person.Id, child.RelationshipType.Value );
                                newRelationships.AddOrIgnore( person.Id, new List<int>() );
                                //newRelationships[person.Id].Add( newRelationshipId.Value );
                                newRelationships[person.Id].Add( child.RelationshipType.Value );
                            }
                        }
                    }
                    else
                ...
                if ( workflows.Any() || redirectUrl.IsNotNullOrWhiteSpace() )
                {
                    ...
                    parameters.Add( "CampusId", ( cpCampus.SelectedCampusId ?? family.CampusId ).ToString() );

                    if ( pnlPlannedDate.Visible )
                    {

                        string enroll = ddlVisitingEnroll.SelectedValue;
                        if ( enroll.IsNotNullOrWhiteSpace() )
                        {
                            parameters.Add( "EnrollFamily", enroll.EncodeHtml() );
                        }
                    }
        ...
        private void SetControls()
        {
            ...
            ddlVisitingEnroll.Required = SetControl( "EnrollFamily", pnlPlannedDate, null );
        ...
        private void SetCurrentFamilyValues()
        {
            ...
            // If a Person ID is passed as a page parameter, update family from it. (Should not be used on public site)
            if ( GetAttributeValue( "PersonIdPageParameter" ).AsBoolean() && PageParameter("PersonId").IsNotNullOrWhiteSpace() )
            {
                Person spouse = null;
                Person person = null;
                var personFromId = new PersonService( _rockContext ).Get( PageParameter( "PersonId" ).AsInteger() );
                
                // Get all their families
                var families = personFromId.GetFamilies( _rockContext );
                if ( families.Any() )
                {
                    // Get Primary person in family, from selected personFromId
                    person = personFromId.GetHeadOfHousehold( _rockContext );

                    // Get their spouse
                    spouse = person.GetSpouse( _rockContext );
                    if ( spouse != null )
                    {
                        // If spouse was found, find the first family that spouse belongs to also.
                        family = families.Where( f => f.Members.Any( m => m.PersonId == spouse.Id ) ).FirstOrDefault();
            ...
            Children = new List<PreRegistrationChildNPM>();
            ...
            if ( family != null )
            {
                ...
                {
                    var child = new PreRegistrationChildNPM( groupMember.Person );
                    child.RelationshipType = 0;

                    //Add in attributevalues from person
                    var attributeKeys = GetCategoryAttributeList( CHILD_CATEGORIES_KEY ).Select( a => a.Key ).ToList();
                    groupMember.Person.LoadAttributes();
                    child.AttributeValues = groupMember.Person.AttributeValues
            ...
            else
            {
                ...
                else if ( PageParameter("CampusId" ).AsIntegerOrNull().HasValue ) //Created to accept CampusId as a default
                {
        ...
        private void BuildFamilyAttributes( bool setValues, Group family )
        {
            ...
            foreach ( var attribute in attributeList )
            {
                ...
                attribute.AddControl( phFamilyAttributes.Controls, value, this.BlockValidationGroup, setValues, true, /*attribute.IsRequired*/ true );
            }
        }
        ...
        private void AddChild()
        {
            ...
            var child = new PreRegistrationChildNPM( person );
        ...
        private void CreateChildrenControls( bool setSelection )
        {
            ...
            foreach ( var child in Children )
            {
                if ( child != null )
                {
                    var childRow = new PreRegistrationChildRowNPM();
                    ...
                    if ( setSelection )
                    {
                        ...
                        childRow.SetAttributeValues( child ); //not sure that does the trick?
            ...
            DynamicPlaceholder phAttributes )
        {
            ...
            if ( adult != null || ( tbFirstName.Text.IsNotNullOrWhiteSpace() && tbLastName.Text.IsNotNullOrWhiteSpace() ) )
            {
                ...
                {
                    string key = AttributeCache.Get( GetAttributeValue( "ProfileSourceAttribute" ).AsGuid() ).Key;
                    if (adult.GetAttributeValue( key ).IsNullOrWhiteSpace())
                    {
                        adult.SetAttributeValue( key, "New Family Registration Form" );
                    }
                }
        ...
        private List<AttributeCache> GetCategoryAttributeList( string attributeKey )
        {
            ...
            foreach ( Guid categoryGuid in GetAttributeValue( attributeKey ).SplitDelimitedValues( false ).AsGuidList() )
            {
                ...
                if ( category != null )
                {
                    foreach ( var attribute in new AttributeService( _rockContext ).GetByCategoryId( category.Id, false ).OrderBy( a => a.Order ) )
                    {
        ...
        private Group CreateNewFamily( int familyGroupTypeId, string lastName )
        {
            ...
            else if ( PageParameter("CampusId").IsNotNullOrWhiteSpace() )
            {
                family.CampusId = int.Parse( PageParameter( "CampusId" ) );
            }
            else
            {
                Guid? campusGuid = GetAttributeValue( "DefaultCampus" ).AsGuidOrNull();
