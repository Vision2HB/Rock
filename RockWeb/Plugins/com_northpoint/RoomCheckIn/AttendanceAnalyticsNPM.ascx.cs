...
using com_northpoint.RoomCheckIn.Data;
...
namespace RockWeb.Plugins.com_northpoint.RoomCheckIn
{
    ...
    /// </summary>
    [DisplayName( "Attendance Analytics NPM" )]
    ...
    public partial class AttendanceAnalyticsNPM : RockBlock
    {
...
";
                ...
                if ( showAllGroups )
                {
                    ...
                    if ( !selectedGroupIds.Any() )
                    {
                        foreach( var cblGroup in checkboxListControls )
                        {
                            ...
                            {
                                selectedGroupIds.Add( item.Value.AsInteger() );
                            }
                        }
                    }
                }
        ...
        private IEnumerable<Rock.Chart.IChartData> GetAttendanceChartData()
        {
            ...
            List<string> groupTypeRoles = cblGroupRoleType.SelectedValues;
            
            var chartData = AttendanceAnalyticsNPMHelper.GetChartData( groupBy, graphBy, start, end, groupIds, campusIds, dataView, scheduleIds, groupTypeRoles );
        ...
        private void BindAttendeesGrid( bool isExporting = false )
        {
            ...
            List<string> groupTypeRoles = cblGroupRoleType.SelectedValues;
            List<int> groupLeadersPersonIds = new List<int>();
            List<int> groupNonLeadersPersonIds = new List<int>();
            //Check For Group Role Types
            if (groupTypeRoles.Any() && groupTypeRoles.Count() > 0 && groupTypeRoles.Count() < 3)
            {
                // Get group members and leaders
                qryTasks.Add(Task.Run(() =>
                {
                    var ti = new TaskInfo { name = "Get Group Member Records", start = DateTime.Now };
                    taskInfos.Add(ti);
                    using (RockContext context = new RockContext())
                    {
                        groupLeadersPersonIds = new GroupMemberService( context ).Queryable("Group,GroupRole").AsNoTracking()
                          .Where(gm => groupIdList.Contains(gm.GroupId))
                          .Where(gm => gm.GroupRole.IsLeader == true)
                          .Where(gm => gm.Group.InactiveDateTime == null || gm.Group.InactiveDateTime > start)
                          .Where(gm => gm.Group.ArchivedDateTime == null || gm.Group.ArchivedDateTime > start)
                ...
                qryTasks.Add(Task.Run(() =>
                {
                    var ti = new TaskInfo { name = "Get Group Leader Records", start = DateTime.Now };
                    taskInfos.Add(ti);

                    using (RockContext context = new RockContext())
                    {
                        groupNonLeadersPersonIds = new GroupMemberService( context ).Queryable("Group,GroupRole").AsNoTracking()
                            .Where(gm => groupIdList.Contains(gm.GroupId))
                            .Where(gm => gm.GroupRole.IsLeader != true)
                            .Where(gm => gm.Group.InactiveDateTime == null || gm.Group.InactiveDateTime > start)
                            .Where(gm => gm.Group.ArchivedDateTime == null || gm.Group.ArchivedDateTime > start)
            ...
            if( groupTypeRoles.Any() && groupTypeRoles.Count() > 0 && groupTypeRoles.Count() < 3 )
            {
                List<int> peopleIdsToKeep = new List<int>();

                //First, if No Record selected, 

                if( groupTypeRoles.Contains( "Member" ) )
                {
                    //If Member is selected, add to keep range
                    //peopleIdsToKeep.AddRange( allResults.Where( ar => groupNonLeadersPersonIds.Contains( ar.PersonId ) ).Select( ar => ar.PersonId ).ToList() );
                    peopleIdsToKeep.AddRange( groupNonLeadersPersonIds );
                }
                if ( groupTypeRoles.Contains( "Leader" ) )
                {
                    //If Leader is selected, add to keep range
                    //peopleIdsToKeep.AddRange( allResults.Where( ar => groupLeadersPersonIds.Contains( ar.PersonId ) ).Select( ar => ar.PersonId ).ToList() );
                    peopleIdsToKeep.AddRange( groupLeadersPersonIds );
                }
                if ( groupTypeRoles.Contains( "NoRecord" ) )
                {
                    //If No Record is selected, add to keep range
                    List<int> gm = new List<int>();
                    gm.AddRange( groupNonLeadersPersonIds );
                    gm.AddRange( groupLeadersPersonIds );
                    peopleIdsToKeep.AddRange( allResults.Where( ar => !gm.Contains( ar.PersonId ) ).Select( ar => ar.PersonId ).ToList() );
                }

                //keep only people selected
                allResults = allResults.Where( ar => peopleIdsToKeep.Contains( ar.PersonId ) ).ToList();
            }
