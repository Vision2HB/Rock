...
{
    [DisplayName("Person Select (Family Check-in) with Remove")]
...
", person.Person.FullName );
        ...
        private void RSelection_ItemCommand( object source, RepeaterCommandEventArgs e )
        {
            ...
            {
                ShowRemoveDialog( e.CommandArgument.ToString().AsInteger() );
            }
        }
...
", person.Person.FullName );
        ...
        private void ShowRemoveDialog( int personId )
        {
            var family = CurrentCheckInState.CheckIn.CurrentFamily;
            if ( family == null )
            {
            ...
            if ( person != null )
            {
                hfRemoveRelationshipPersonId.Value = person.Person.Id.ToString();
                //Show Dialog to confirm removal
                mdlConfirmRemoveRelationship.Show();
            }
        }
        ...
        protected void mdlConfirmRemoveRelationship_SaveClick( object sender, EventArgs e )
        {
            //Get person Id and remove relationship from current family
            
            var rockContext = new RockContext();
            var personService = new PersonService( rockContext );
            Person removeRelationshipsPerson = personService.Get( hfRemoveRelationshipPersonId.Value.AsInteger() );
            var familyIds = new GroupMemberService( rockContext ).GetByGroupId( CurrentCheckInState.CheckIn.CurrentFamily.Group.Id ).Where( p => p.Person.Id != removeRelationshipsPerson.Id ).Select( p => p.Person.Id ).ToList();
            
            if ( removeRelationshipsPerson.IsNotNull() )
            {
                var knownRelationshipsGuid = Rock.SystemGuid.GroupType.GROUPTYPE_KNOWN_RELATIONSHIPS.AsGuid();
                var canCheckInGUID = Rock.SystemGuid.GroupRole.GROUPROLE_KNOWN_RELATIONSHIPS_CAN_CHECK_IN.AsGuid();
                var allowCheckInGUID = Rock.SystemGuid.GroupRole.GROUPROLE_KNOWN_RELATIONSHIPS_ALLOW_CHECK_IN_BY.AsGuid();
                var ownerGUID = Rock.SystemGuid.GroupRole.GROUPROLE_KNOWN_RELATIONSHIPS_OWNER.AsGuid();
                
                //var groupService = new GroupService( rockContext );
                var groupMemberService = new GroupMemberService( rockContext );

                //Find relationships of the remove person as a can check in type
                var relationships = groupMemberService.Queryable().Where( gm => gm.PersonId == removeRelationshipsPerson.Id && gm.Group.IsActive && gm.Group.GroupType.Guid == knownRelationshipsGuid )
                        .Where( gm => gm.GroupRole.Guid == canCheckInGUID );

                //pair down the list to those whose owners are of the adults of this family
                var familyOwnerRelationships = groupMemberService.Queryable().Where( gm => gm.Group.GroupType.Guid == knownRelationshipsGuid )
                    .Where( gm => familyIds.Contains( gm.PersonId ) && gm.GroupRole.Guid == ownerGUID )
                    .Select( gm => gm.GroupId ).ToList();
                
                var canCheckInRelationshipGroupMembers = relationships.Where( gm => familyOwnerRelationships.Contains( gm.GroupId ) ).ToList();

                //for each canCheckIn relationship, remove the inverse and then itself
                for ( int i = 0; i < canCheckInRelationshipGroupMembers.Count(); i++ )
                {
                    var relationship = canCheckInRelationshipGroupMembers[i];
                    var inverse = groupMemberService.GetInverseRelationship( relationship, false );
                    if ( inverse != null )
                    {
                        var deletedInverse = groupMemberService.Delete( inverse );
                    }

                    var deleted = groupMemberService.Delete( relationship );
                }
                
                rockContext.SaveChanges();

                //remove person from currentcheckin family
                CurrentCheckInState.CheckIn.CurrentFamily.People.RemoveAll( p => p.Person.Id == removeRelationshipsPerson.Id );
                SaveState();
            }
