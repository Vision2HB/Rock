...
namespace RockWeb.Plugins.com_northpoint.RoomCheckIn
{
    ...
    [Description( "A directory of all people in database." )]
    ...
    public partial class PersonDirectory : Rock.Web.UI.RockBlock
    {
        ...
        private void rptPeople_ItemDataBound( object sender, RepeaterItemEventArgs e )
        {
            ...
            if ( personItem != null )
            {
                ...
                if ( lPerson != null )
                {
                    ...
                    personName = string.IsNullOrWhiteSpace( _personProfileUrl ) ? personName : string.Format( "<a href='{0}{1}'>{2}</a>", string.Format(_personProfileUrl, personItem.Id ), PageParameter("CampusId").IsNotNullOrWhiteSpace() ? "&CampusId=" + PageParameter("CampusId") : "" , personName );
        ...
        {
            NavigateToLinkedPage( "PersonProfilePage", new Dictionary<string, string> { { "CampusId", PageParameter( "CampusId" ) } } );
        }
        ...
        private void BindData()
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                var personQry = personService.Queryable( false, false ).AsNoTracking();
                ...
                // Filter by phone
                string phoneNumber = tbPhone.Text.Trim().RemoveAllNonAlphaNumericCharacters();
                if ( !string.IsNullOrWhiteSpace( phoneNumber ) )
                {
                    var phonePersonIds = new PhoneNumberService( rockContext ).Queryable().AsNoTracking()
                        .Where( n => n.Number.Contains( phoneNumber ) ).Select( n => n.PersonId ).ToList();

                    personQry = personQry.Where( p =>
                        phonePersonIds.Contains( p.Id )  );
                    filteredQry = true;
                }
