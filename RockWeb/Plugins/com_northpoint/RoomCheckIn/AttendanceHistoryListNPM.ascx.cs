...
namespace RockWeb.Plugins.com_northpoint.RoomCheckIn
{
    ...
    /// </summary>
    [DisplayName( "Attendance History NPM" )]
    ...
    [ContextAware]
    public partial class AttendanceHistoryListNPM : RockBlock, ICustomGridColumns
    {
        #region Fields
        ...
        protected void gHistory_RowDataBound( object sender, GridViewRowEventArgs e )
        {
            ...
            if ( dataItem != null )
            {
                ...
                Literal lCode = e.Row.FindControl( "lCode" ) as Literal;
                if ( lCode != null )
                {
                    lCode.Text = string.Format( "{0}", dataItem.GetPropertyValue( "Code" ) ?? "" );
                }

                Literal lNote = e.Row.FindControl( "lNote" ) as Literal;
                if ( lNote != null )
                {
                    string note = ( dataItem.GetPropertyValue( "Note" ) as string ) ?? "";
                    note = note.TrimStart( '|' ).Replace( "|", "<br />" ).Replace( "-InRoom Location Change", "" ).Trim();
                    lNote.Text = string.Format( "<small>{0}</small>", note );
                }
            }
        }
