    ...
    public partial class KioskTypeDetail : RockBlock, IDetailBlock
    {
        ...
        protected void btnSave_Click( object sender, EventArgs e )
        {
            ...
            if ( KioskType != null )
            {
                ...
                KioskType.CheckinTheme = tbTheme.Text;
                ...
                //Set Page Redirect Id
                if ( pageRedirect.SelectedValue.IsNotNullOrWhiteSpace() )
                {
                    KioskType.RedirectPageId = pageRedirect.SelectedValueAsId();
                }
                ...
                //Add in additional areas
                foreach ( ListItem item in cblAdditionalGroupTypes.Items )
                {
                    if ( item.Selected )
                    {
                        GroupTypes.Add( groupTypeService.Get( item.Value.AsInteger() ) );
        ...
        public void ShowDetail( int kioskTypeId )
        {
            ...
            tbTheme.Text = KioskType.CheckinTheme;
            //Set Page Redirect
            pageRedirect.SetValue( KioskType.RedirectPageId );
        ...
        private void BindGroupTypes( string selectedValues )
        {
            ...
            cblAdditionalGroupTypes.Items.Clear();
            ...
            //Bind Additional areas (configs not in selected primary)
            Guid templateTypeGuid = Rock.SystemGuid.DefinedValue.GROUPTYPE_PURPOSE_CHECKIN_TEMPLATE.AsGuid();
            var otherCheckInConfigs = groupTypeService
                .Queryable().AsNoTracking()
                .Where( t =>
                    t.GroupTypePurposeValue != null &&
                    t.GroupTypePurposeValue.Guid == templateTypeGuid )
                .Where( t => t.Id != templateGroupType.Id ) //not the selected template
                .OrderBy( t => t.Name )
                .ToList();
            var additionalGroups = new List<GroupType>();
            foreach (var groupType in otherCheckInConfigs )
            {
                additionalGroups.AddRange( GetPrimaryGroupTypesFromTemplate( groupType ) );
            }
            cblAdditionalGroupTypes.DataSource = additionalGroups.DistinctBy( t => t.Id ).OrderBy( t => t.Name ).ToList();
            cblAdditionalGroupTypes.DataBind();
            ...
            if ( selectedValues != string.Empty )
            {
                foreach ( string id in selectedValues.Split( ',' ) )
                {
                    ...
                    item = cblAdditionalGroupTypes.Items.FindByValue( id );
                    if ( item != null )
                    {
                        item.Selected = true;
                    }
        ...
        private List<GroupType> GetPrimaryGroupTypesFromTemplate( GroupType templateGroupType )
        {
            ...
            foreach ( var groupType in primary )
            {
                ...
                if ( children.Any() )
                {
                    ...
                    {
                        if ( child.ChildGroupTypes.Any() && !primaryGroupTypes.Contains( child ) )
                        {
                            //primaryGroupTypes.AddRange( child.ChildGroupTypes.ToList() );
                            primaryGroupTypes.AddRange( GetPrimaryGroupTypesFromTemplate( child ) );
                        }
                        else
                        {
            ...
            return primaryGroupTypes.DistinctBy( gt => gt.Id ).ToList();
        }
