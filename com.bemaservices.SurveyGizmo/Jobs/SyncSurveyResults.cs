        ...
        public virtual void Execute( IJobExecutionContext context )
        {
            ...
            if ( apiToken.IsNotNullOrWhitespace() && apiTokenSecret.IsNotNullOrWhitespace() )
            {
                using ( var rockContext = new RockContext() )
                {
                    ...
                    int definedTypeId = DefinedTypeCache.Read( com.bemaservices.SurveyGizmo.SystemGuid.DefinedType.SURVEY_GIZMO_SURVEYS.AsGuid() ).Id;
                    ...
                    else
                    {
                        foreach ( DefinedValue survey in dvSurveys )
                        {
                            ...
                            {
                                personAttribute = AttributeCache.Read( personAttributeGuid.Value );
                            }
                                                        ...
                                                        if ( person != null )
                                                        {
                                                            ...
                                                            if ( attributeMatrix != null )
                                                            {
                                                                foreach ( var matrixItem in attributeMatrix.AttributeMatrixItems )
                                                                {
                                                                    ...
                                                                    {
                                                                        matrixPersonAttribute = AttributeCache.Read( matrixPersonAttributeGuid.Value );
                                                                    }
